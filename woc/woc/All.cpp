...
#pragma region Monobehavior £º Object
...
void Transform::OnGUI() const
{
	...
	ImGui::DragFloat3((gameObject->name + "_scale").c_str(), (float*)&scanle, 0.01f, -100, 100);
...
void Camera::Start()
{
	...
	cout << viewPort.z << " " << viewPort.w << endl;
}
...
#pragma region AbstractLight £ºMonoBehavior
...
void AbstractMaterial::Use(mat4 & view, mat4 & proj, mat4 model, float skininess)
{
	shader->use();
...
void StandandMaterial::Use(mat4 & view, mat4 & proj, mat4 model, float skininess)
{
	AbstractMaterial::Use(view, proj, model, skininess);
...
void BoxMaterial::Use(mat4 & view, mat4 & proj, mat4 model, float skininess)
{
	AbstractMaterial::Use(view, proj, model, skininess);
...
{
	material->Use(viewMat, projMat, gameObject->transform()->GetModelMaterix(), 32.0f);
...
void SkyboxRender::RealUpdate()
{
	...
	auto x = gameObject->transform()->scanle;
